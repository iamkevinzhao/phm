...
sdv_state = [0.01 0.01 0.01]';
global noise_state; % noises over time on system states
if gen_new == 1
    noise_state = ...
        normrnd(zeros(3, cyc_total), repmat(sdv_state, 1, cyc_total));
...
sdv_msu = [0.01 0.01 0.01 0.01 0.01]';
global noise_msu; % noises over time on measurements
if gen_new == 1
    noise_msu = ...
        normrnd(zeros(5, cyc_total), repmat(sdv_msu, 1, cyc_total));
